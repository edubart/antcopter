...
WASM_OPT = wasm-opt
WASM_OPT_FLAGS = -Oz --zero-filled-memory --strip-producers --strip-dwarf
...
all:
	@mkdir -p build
	$(NELUA) $(NELUA_FLAGS) src/antcopter.nelua --output build/cart.wasm
ifneq ($(DEBUG), 1)
...
bundle:
	w4 bundle build/cart.wasm --html docs/index.html --title Antcopter
